...
DAC_MCP49xx::DAC_MCP49xx(DAC_MCP49xx::Model _model, int _sdi_pin, int _sdo_pin, int _sck_pin, int _ss_pin, int _LDAC_pin) : bufferVref(false), gain2x(false), port_write(false), spi_divider(SPI_CLOCK_DIV2), automaticallyLatchDual(true) {
  this->ss_pin = _ss_pin;
  ...
  */
  ...
  SSPI.begin(_sdi_pin, _sdo_pin, _sck_pin);
  SSPI.setBitOrder(MSBFIRST);
  SSPI.setDataMode(SPI_MODE0);
  SSPI.setClockDivider(spi_divider);
}
...
void DAC_MCP49xx::shutdown(void) {
  ...
  SSPI.transfer((out & 0xff00) >> 8);
  SSPI.transfer(out & 0xff);
